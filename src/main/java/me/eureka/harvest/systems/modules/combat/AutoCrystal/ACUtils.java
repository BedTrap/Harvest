...
public class ACUtils {
    public static List<BlockPosX> getSphere(PlayerEntity player, double radius) {
        ...
        return blocks.stream().filter(bp -> !bp.air()).toList();
    }
    ...
    public static EndCrystalEntity getEntity(BlockPosX blockPos) {
        ...
        return hasEntity(new Box(blockPos), entity -> entity instanceof EndCrystalEntity) ?
                (EndCrystalEntity) Wrapper.mc.world.getOtherEntities(null, new Box(blockPos), entity -> entity instanceof EndCrystalEntity).get(0) : null;
    }
    ...
    public static double distanceTo(double x, double y, double z) {
        Vec3d eyePos = new Vec3d(Wrapper.mc.player.getX(), Wrapper.mc.player.getY() + Wrapper.mc.player.getEyeHeight(Wrapper.mc.player.getPose()), Wrapper.mc.player.getZ());
    ...
    public static void processAttack(Entity entity, String attack, Vec3d hitPos) {
        switch (attack) {
            case "Packet" -> {
                Wrapper.mc.getNetworkHandler().sendPacket(PlayerInteractEntityC2SPacket.attack(entity, Wrapper.mc.player.isSneaking()));
                Wrapper.mc.getNetworkHandler().sendPacket(new HandSwingC2SPacket(Hand.MAIN_HAND));
            }
            case "Client" -> Wrapper.mc.interactionManager.attackEntity(Wrapper.mc.player, entity);
            case "Vanilla" -> {
                HitResult prevResult = Wrapper.mc.crosshairTarget;
                Wrapper.mc.crosshairTarget = new EntityHitResult(entity, hitPos);
                leftClick();
                Wrapper.mc.crosshairTarget = prevResult;
            }
        }
    }

    public static void leftClick() {
        Wrapper.mc.options.attackKey.setPressed(true);
        ((MinecraftClientAccessor) Wrapper.mc).leftClick();
        Wrapper.mc.options.attackKey.setPressed(false);
    }
    ...
    public static List<BlockPosX> getSurroundBlocks(PlayerEntity player, boolean allBlocks) {
        ...
        for (BlockPosX blockPos : getSphere(new BlockPosX(player.getBlockPos()), 3, 1)) {
            if (!allBlocks && !Wrapper.mc.world.getBlockState(blockPos).getMaterial().isReplaceable()) continue;
    ...
    public static OtherClientPlayerEntity predictedTarget(PlayerEntity player) {
        OtherClientPlayerEntity fakeTarget = new OtherClientPlayerEntity(Wrapper.mc.world, new GameProfile(player.getUuid(), "[Predict] " + player.getGameProfile().getName()), player.getPublicKey());
    ...
    public static void refill(FindItemResult from, int to) {
        move(to, from.slot());
    }

...
public class BlockPosX extends BlockPos {
    public static BlockPosX feetPos() {
        return new BlockPosX(mc.player.getBlockPos());
    }
    ...
    public Box renderBox() {
        Vec3d v = Renderer3D.getRenderPosition(this.vec3d());
        return new Box(v.x, v.y, v.z, v.x + 1.0, v.y + 1.0, v.z + 1);
    }
    ...
    // Entity
    // This method will check twice for an entity, useful for breaking crystals and then placing blocks
    public boolean hasEntity(Consumer<Entity> consumer) {
        boolean has_entity = hasEntity();

        if (has_entity) {
            List<Entity> entity = getEntities();
            consumer.accept(entity.get(0));
            return hasEntity();
        }
    ...
    public double crystalDamage(PlayerEntity player) {
        return DamageManager.crystalDamage(mc.player, this.vec3d().add(0, 0.5, 0));
    }
    public Direction closestDirection() {
        ...
        for (Direction direction : Direction.values()) {
            Vec3d offset = new Vec3d(
                    this.getX() + 0.5 + direction.getVector().getX() * 0.5,
                    this.getY() + 0.5 + direction.getVector().getY() * 0.5,
                    this.getZ() + 0.5 + direction.getVector().getZ() * 0.5
            );

            if (eyePos.distanceTo(offset) < distance) {
                bestDirection = direction;
                distance = eyePos.distanceTo(offset);
            }
        }

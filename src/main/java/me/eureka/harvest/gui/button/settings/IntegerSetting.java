...
public class IntegerSetting extends SettingButton {
    ...
    public void render(int mouseX, int mouseY) {
        ...
        Hack.gui().drawFlat(x() + 3, y() + ClickGUI.INSTANCE.sliderHeight.get(), (int) (x() - 2 + (sliderWidth) + 5), y() + h(), hover ? HUD.INSTANCE.colorType().darker().getRGB() : HUD.INSTANCE.colorType().getRGB());

        mc.textRenderer.drawWithShadow(stack, setting.getName(), (float) (x() + 6), (float) (y() + 4), WHITE);
        mc.textRenderer.drawWithShadow(stack, String.valueOf(setting.get()), (float) ((x() + 6) + mc.textRenderer.getWidth(" " + setting.getName())), (float) (y() + 4), WHITE);
    }
    ...
    public void mouseDown(int mouseX, int mouseY, int button) {
        if (!ClickGUI.INSTANCE.isActive() || button == 1) {
            dragging = false;
            return;
        }
    ...
    public static class Slider extends IntegerSetting {
        ...
        protected void updateSlider(int mouseX) {
            ...
            if (dragging) {
                ...
                Hack.event().post(new StateEvent.Number(this.module(), setting));
            }
        }
    }
}
